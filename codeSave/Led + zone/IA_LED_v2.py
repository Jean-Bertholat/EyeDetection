...
#GPIO.setwarnings(False) #On désactive les messages d'alerte
GPIO_LED = [[37,33,11,7],[40,38,18,16],[32, 31,29,22]] #Définit le numéro du port GPIO qui alimente la led
XY_AREAS = [4, 3]
...
def getLocation(listCoor):
    if listCoor == []:
        return None
    
    elif len(listCoor) == 1:
        return listCoor[0] #if only one eye we only treat one for the moment
    
    elif len(listCoor) == 2:
        return ((listCoor[0][0] + listCoor[1][0]) //2), ((listCoor[0][1] + listCoor[1][1]) //2)
    
    else:
        raise TypeError('expected at most 2 arguments') #If more than 2 eyes on the screen raise error
...
def ledManag(frame, listEyes, listLed):
    ...
    try:
        ...
        for i in range (len(X)-1):
            if (center[0] > X[i]) and (center[0] < X[i+1]):
                
                for j in range (len(Y)-1):
                    if center[1] > Y[j] and center[1] < Y[j+1]:
                        
                        GPIO.output(listLed[j][i], GPIO.HIGH) #On allume la bonne led
                        
    except IndexError:
         print("Pas d'yeux détectés") # if no eyes detected listEyes is empty, null
